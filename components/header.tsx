          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navigation.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="font-medium transition-colors duration-200 text-neutral-700 hover:text-brand-blue"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {user ? (
              <div className="flex items-center space-x-3">
                <span className="text-sm text-neutral-600">Olá, {user.email}</span>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={signOut}
              ...
              <>
                ...
                <Link href="/contact">
                  <Button className="btn-primary">{t("navigation.startProject")}</Button>
                </Link>
              </>
            )}
          </div>
            ...
            <nav className="flex flex-col space-y-4">
              {navigation.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="px-2 py-1 font-medium transition-colors duration-200 text-neutral-700 hover:text-brand-blue"
              ...
              <div className="pt-4 flex flex-col space-y-2">
                ...
                {user ? (
                  <>
                    <span className="text-sm text-neutral-600 px-2">Olá, {user.email}</span>
                    <Button
                      variant="outline"
                      onClick={signOut}
                  ...
                  <>
                    ...
                    <Link href="/contact">
                      <Button className="btn-primary w-full">{t("navigation.startProject")}</Button>
                    </Link>
                  </>
                )}
              </div>
